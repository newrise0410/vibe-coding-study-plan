...
아래 12장은 **커리큘럼 본문에 자리가 잡혀 있는 것들**이다. 운영자가 웹을 만들며 찍을
전체 목록(62장)은 `docs/study-web/SHOOTING-GUIDE.md`에 있고, 이 12장은 그 일부다.
...
운영자가 이 스터디를 굴리며 직접 겪는 화면들이라, **무엇이 남았는지 세는 체크리스트는
`docs/study-web/SHOOTING-GUIDE.md`** 한 곳에서 관리한다. 위 12장도 거기 📌 로 표시돼 있다.
이 문서는 파일을 어디에 어떤 이름으로 넣는지만 다룬다.
