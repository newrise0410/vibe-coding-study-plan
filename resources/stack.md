...
### Orca
AI 에이전트를 돌리는 IDE다. **에이전트는 직접 주지 않는다** — 쓰던 걸 가져다 붙인다(BYO).

이 스터디가 Orca를 쓰는 이유는 셋이다.

| 기능 | 이 스터디에서 |
|---|---|
| **Attribution** | diff에서 **AI가 쓴 줄**이 거터에 표시된다. `해부하기`의 출발점 |
| **워크트리** | 작업 하나에 폴더 하나. 망가뜨려도 버리면 끝이라 `부숴보기`가 안전하다 |
| **워크트리별 브라우저** | 미리보기가 작업마다 격리된다 |

에이전트는 각자 고른다. Gemini CLI는 무료 티어가 있고, Claude Code·Codex·Cursor CLI는 구독이 필요하다.
**커리큘럼은 특정 에이전트에 기대지 않는다.**
